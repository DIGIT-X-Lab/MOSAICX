...
The extract pipeline provides flexible document extraction in two modes: Auto mode (infers schema from document) and Template mode (extracts into a provided YAML template).
...
# Template mode: extract using a saved template
mosaicx extract --document report.pdf --template PatientIntakeForm
```
...
**Auto mode** (no template provided):
...
**Template mode** (template provided):
1. **Extract** — Single-step ChainOfThought extraction directly into the Pydantic model compiled from the YAML template.
...
**Template mode:**
- `extracted` (dict or Pydantic model) — Extracted data matching the provided template
...
### Example JSONL Record (Template Mode)

For template mode, you would first create a YAML template using `mosaicx template create`. The JSONL record follows the same pattern but the `extracted` field must match your specific template structure.
...
The template pipeline generates extraction templates from natural language descriptions, enabling you to create custom extraction templates without writing code. Templates are stored as YAML files in `~/.mosaicx/templates/`.

### CLI Commands

```bash
# Create template from description
mosaicx template create --description "A patient intake form with demographics, allergies, medications, and social history"

# Create template from example document
mosaicx template create --from-document example.pdf --name PatientIntake

# Refine an existing template
mosaicx template refine PatientIntake "Add a field for emergency contact phone number"

# List all saved templates
mosaicx template list

# Show template details
mosaicx template show PatientIntake
```
...
| **Extract** | `mosaicx extract [--template NAME]` | `document_text` | `extracted`, optionally `inferred_schema` |
...
| **Template** | `mosaicx template create` | `description`, `example_text`, `document_text` | `schema_spec`, `compiled_model` |
